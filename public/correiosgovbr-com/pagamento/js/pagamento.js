...
(function() {
    ...
    async function gerarPix() {
        ...
        if (transacaoExistente) {
            ...
            try {
                const statusResponse = await fetch(`${CONFIG.API_STATUS_ENDPOINT}?hash=${encodeURIComponent(transacaoExistente.hash)}`);
        ...
        try {
            ...
            const response = await fetch(CONFIG.API_PIX_ENDPOINT, {
                method: 'POST',
                headers: {
    ...
    async function verificarStatus() {
        ...
        try {
            const response = await fetch(`${CONFIG.API_STATUS_ENDPOINT}?hash=${encodeURIComponent(state.transactionHash)}`);
            if (!response.ok) {
                throw new Error(`Erro do servidor (${response.status})`);
            }
            const result = await response.json();
